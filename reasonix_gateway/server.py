#!/usr/bin/env python3
"""HTTP server layer for the claude-reasonix native gateway.

Top layer of the reasonix_gateway package: the Anthropic Messages-compatible
``Handler``, the ``_keepalive_loop`` background thread, and ``main`` that boots
the ``ThreadingHTTPServer``. Everything here consumes the lower layers
(env, text, levers, cost, engine_seam); nothing in the package imports server.
"""
...
from .env import JSON, env_first, env_float
from .text import json_bytes
from .levers import (
    _keepalive_enabled, gateway_trace, keepalive_targets, load_read_cache,
    model_registry,
)
from .engine_seam import (
    GatewayError,
    estimate_tokens, call_openai_compatible, call_openai_chat_completion,
    run_reasonix_acp,
)
